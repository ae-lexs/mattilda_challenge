...
│       └── account_statement_cache.py # NEW: Cache port interfaces
├── infrastructure/
...
│   └── adapters/
│       ├── __init__.py
│       ├── invoice_repository.py      # Existing
│       ├── student_account_statement_cache.py  # NEW: Cache adapter
│       └── school_account_statement_cache.py   # NEW: Cache adapter
```
...
```python
# infrastructure/adapters/student_account_statement_cache.py
from __future__ import annotations
...
class RedisStudentAccountStatementCache(StudentAccountStatementCache):
    ...
    async def set(self, statement: StudentAccountStatement) -> None:
        ...
        try:
            ...
            logger.debug("cache_set", key=key, ttl=self._ttl)
    ...
    def _serialize(self, statement: StudentAccountStatement) -> str:
        ...
        Uses string representation for Decimal values to preserve precision.
        """
        return json.dumps({
    ...
    def _deserialize(self, json_str: str) -> StudentAccountStatement:
        ...
        Converts string decimals back to Decimal type.
        """
...
```python
# infrastructure/adapters/school_account_statement_cache.py
from __future__ import annotations
...
- [ ] Add `redis-py` to dependencies in `pyproject.toml`
- [ ] Add Redis service to `docker-compose.yml`
- [ ] Create `config.py` entries for `REDIS_URL` and `CACHE_TTL_SECONDS`
- [ ] Create `infrastructure/redis/client.py` with connection pool
- [ ] Add Redis pool cleanup to FastAPI lifespan handler

### Phase 2: Ports and Adapters
- [ ] Create `application/ports/account_statement_cache.py` with ABC interfaces
- [ ] Create `infrastructure/adapters/student_account_statement_cache.py`
- [ ] Create `infrastructure/adapters/school_account_statement_cache.py`
- [ ] Create `infrastructure/adapters/null_account_statement_cache.py` for testing
...
### Phase 5: Testing
- [ ] Unit tests for cache adapters (mock Redis client)
- [ ] Unit tests for use cases with null cache (verify fallback)
- [ ] Integration tests with real Redis (verify TTL behavior)
- [ ] Test fail-open behavior (Redis unavailable scenario)

### Phase 6: Documentation
- [ ] Update README with Redis setup instructions
- [ ] Add cache configuration to environment variables documentation
- [ ] Document cache inspection commands (`redis-cli KEYS "mattilda:cache:*"`)
...
**Note**: Log statements use structured logging style (e.g., `logger.debug("cache_hit", key=key)`). This assumes a logging configuration that supports keyword arguments, such as `structlog` or JSON-formatted logging. If using stdlib `logging` with default formatter, adapt to f-string style: `logger.debug(f"cache_hit key={key}")`.
