...
The adapter directory structure **mirrors** the port structure. Each port has a corresponding adapter folder containing all implementations:
...
└── infrastructure/
    └── adapters/
        ├── __init__.py
        ├── time_provider/             # Existing (restructured)
        │   ├── __init__.py            # Exports: SystemTimeProvider, FixedTimeProvider
        │   ├── system.py              # SystemTimeProvider
        │   └── fixed.py               # FixedTimeProvider (for testing)
        ├── school_repository/         # NEW - mirrors application/ports/school_repository.py
        │   ├── __init__.py            # Exports: PostgresSchoolRepository, InMemorySchoolRepository
        │   ├── postgres.py            # PostgresSchoolRepository
        │   └── in_memory.py           # InMemorySchoolRepository
        ├── student_repository/        # NEW - mirrors application/ports/student_repository.py
        │   ├── __init__.py            # Exports: PostgresStudentRepository, InMemoryStudentRepository
        │   ├── postgres.py            # PostgresStudentRepository
        │   └── in_memory.py           # InMemoryStudentRepository
        ├── invoice_repository/        # NEW - mirrors application/ports/invoice_repository.py
        │   ├── __init__.py            # Exports: PostgresInvoiceRepository, InMemoryInvoiceRepository
        │   ├── postgres.py            # PostgresInvoiceRepository
        │   └── in_memory.py           # InMemoryInvoiceRepository
        └── payment_repository/        # NEW - mirrors application/ports/payment_repository.py
            ├── __init__.py            # Exports: PostgresPaymentRepository, InMemoryPaymentRepository
            ├── postgres.py            # PostgresPaymentRepository
            └── in_memory.py           # InMemoryPaymentRepository
```

**Rationale for mirror structure:**
- **Discoverability**: Finding adapters for a port is trivial (same name, different location)
- **Cohesion**: All implementations of a port live together
- **Scalability**: Adding new implementations (e.g., `redis.py`, `mongodb.py`) is straightforward
- **Clear exports**: Each `__init__.py` explicitly exposes available implementations

### 1.2 Adapter Module Exports

Each adapter folder's `__init__.py` exports all implementations:

```python
# infrastructure/adapters/invoice_repository/__init__.py
...
**INVARIANT**: Only stable, public adapters are exported in `__init__.py`.

| Rule | Description |
|------|-------------|
| **Always use `__all__`** | Explicitly declare public API; prevents accidental exposure of internals |
| **Only export adapter classes** | Do not export helpers, utilities, or implementation details |
| **Stable API only** | If an adapter is experimental or internal, keep it out of `__init__.py` |
| **Import from package, not module** | Consumers should use `from .invoice_repository import X`, not `from .invoice_repository.postgres import X` |

**Why this matters:**
- Refactoring internal helpers won't break consumers
- Clear boundary between public API and implementation details
- `__all__` serves as documentation of the intended public interface
...
# ❌ AVOID: Import from internal module (couples to implementation structure)
from mattilda_challenge.infrastructure.adapters.invoice_repository.postgres import (
    PostgresInvoiceRepository,
)
```

**Exception**: Direct module imports are acceptable in tests when testing the specific implementation's internals, or when the adapter folder's `__init__.py` intentionally doesn't export a class (e.g., experimental adapters).

**Usage:**

```python
# Clean imports from the adapter folder (preferred)
from mattilda_challenge.infrastructure.adapters.invoice_repository import (
    PostgresInvoiceRepository,
    InMemoryInvoiceRepository,
)
...
```python
# infrastructure/adapters/invoice_repository/postgres.py
from __future__ import annotations
...
```python
# infrastructure/adapters/invoice_repository/in_memory.py
from __future__ import annotations
...
- **Discoverability**: Mirror structure makes finding adapters for a port trivial
- **Scalability**: Adding new adapter implementations is straightforward (just add a new file)

### Negative

- **Boilerplate**: Each repository requires port interface + adapter folder + multiple implementation files
- **Duplication**: Filter/sort logic duplicated between PostgreSQL and in-memory implementations
- **Learning curve**: Team must understand port/adapter pattern and directory conventions
...
- [ ] Create `SchoolRepository` ABC in `application/ports/school_repository.py`
- [ ] Create `StudentRepository` ABC in `application/ports/student_repository.py`
- [ ] Create `InvoiceRepository` ABC in `application/ports/invoice_repository.py`
- [ ] Create `PaymentRepository` ABC in `application/ports/payment_repository.py`
...
### Phase 3: Adapter Directory Structure
- [ ] Create `infrastructure/adapters/school_repository/` folder
- [ ] Create `infrastructure/adapters/student_repository/` folder
- [ ] Create `infrastructure/adapters/invoice_repository/` folder
- [ ] Create `infrastructure/adapters/payment_repository/` folder

### Phase 4: PostgreSQL Adapters
- [ ] Implement `PostgresSchoolRepository` in `school_repository/postgres.py`
- [ ] Implement `PostgresStudentRepository` in `student_repository/postgres.py`
- [ ] Implement `PostgresInvoiceRepository` in `invoice_repository/postgres.py`
- [ ] Implement `PostgresPaymentRepository` in `payment_repository/postgres.py`

### Phase 5: In-Memory Adapters
- [ ] Implement `InMemorySchoolRepository` in `school_repository/in_memory.py`
- [ ] Implement `InMemoryStudentRepository` in `student_repository/in_memory.py`
- [ ] Implement `InMemoryInvoiceRepository` in `invoice_repository/in_memory.py`
- [ ] Implement `InMemoryPaymentRepository` in `payment_repository/in_memory.py`

### Phase 6: Adapter Exports
- [ ] Create `school_repository/__init__.py` with exports
- [ ] Create `student_repository/__init__.py` with exports
- [ ] Create `invoice_repository/__init__.py` with exports
- [ ] Create `payment_repository/__init__.py` with exports
- [ ] Update `infrastructure/adapters/__init__.py` to re-export all adapters
